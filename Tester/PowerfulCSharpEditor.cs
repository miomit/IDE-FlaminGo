...
namespace Tester
{
    ...
    {
        string[] keywords = { "writeline", "write", "read", "readline", "scan", "scanline", "new", "ret", "proc", "loop", "enlp", "if", "else", "enif", "fasm", "enfs" };
        string[] methods = { "fl" };
        string[] snippets = { "if ^\n\t#code\nenif", "if ^\n\nelse\n\nenif", "loop ^\n;\nenlp"};
        ...
        Style invisibleCharsStyle = new InvisibleCharsRenderer(Pens.Pink);
        Color currentLineColor = Color.FromArgb(100, 210, 210, 255);
        Color changedLineColor = Color.FromArgb(255, 230, 230, 255);


        public PowerfulCSharpEditor(string[] args)
        {
            InitializeComponent();
        ...
        private void Run_Click(object sender, EventArgs e)
        {
            ...
            if (tsFiles.SelectedItem != null)
            {
                //string locationFile = tsFiles.SelectedItem.Tag as string;
                Save(tsFiles.SelectedItem);

                Scripts.Debug(tsFiles.SelectedItem.Tag as string);


                Process xe;
                xe = new Process();
                xe.StartInfo.FileName = $"{Scripts.DataFile.CURRENT_DERECTORY}{Scripts.DataFile.NAME_PART}.exe";
                xe.StartInfo.Arguments = "";
                xe.Start();
            }
        ...
        private void параметрыToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            p.ShowDialog();
            //MessageBox.Show(Data.LocationCompiler);
        }
    }
