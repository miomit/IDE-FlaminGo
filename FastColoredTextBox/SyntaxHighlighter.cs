...
namespace FastColoredTextBoxNS
{
    public class SyntaxHighlighter : IDisposable
    {
        ...
        protected void PHPAutoIndentNeeded(object sender, AutoIndentEventArgs args)
        {
            ...
            if (Regex.IsMatch(args.PrevLineText, @"^\s*(if|loop|fasm|new|proc|[\}\s]*else)\b[^{]*$"))
        ...
        protected void CSharpAutoIndentNeeded(object sender, AutoIndentEventArgs args)
        {
            ...
            //is unclosed operator in previous line ?
            if (Regex.IsMatch(args.PrevLineText, @"^\s*(if|else|enif|loop|[\}\s]*else)\b[^{]*$"))
                if (!Regex.IsMatch(args.PrevLineText, @"(;\s*$)|(;\s*//)")) //operator is unclosed
                {
                    args.Shift = args.TabLength;
                    return;
...
";
        ...
        protected void InitVBRegex()
        {
            ...
            VBKeywordRegex =
                new Regex(
                    @"\b(write|writeline|read|readline|As|Boolean|ByRef|Byte|ByVal|Call|Case|Catch|CBool|CByte|CChar|CDate|CDbl|CDec|Char|CInt|Class|CLng|CObj|Const|Continue|CSByte|CShort|CSng|CStr|CType|CUInt|CULng|CUShort|Date|Decimal|Declare|Default|Delegate|Dim|DirectCast|Do|Double|Each|Else|ElseIf|End|EndIf|Enum|Erase|Error|Event|Exit|False|Finally|For|Friend|Function|Get|GetType|GetXMLNamespace|Global|GoSub|GoTo|Handles|If|Implements|Imports|In|Inherits|Integer|Interface|Is|IsNot|Let|Lib|Like|Long|Loop|Me|Mod|Module|MustInherit|MustOverride|MyBase|MyClass|Namespace|Narrowing|New|Next|Not|Nothing|NotInheritable|NotOverridable|Object|Of|On|Operator|Option|Optional|Or|OrElse|Overloads|Overridable|Overrides|ParamArray|Partial|Private|Property|Protected|Public|RaiseEvent|ReadOnly|ReDim|REM|RemoveHandler|Resume|Return|SByte|Select|Set|Shadows|Shared|Short|Single|Static|Step|Stop|String|Structure|Sub|SyncLock|Then|Throw|To|True|Try|TryCast|TypeOf|UInteger|ULong|UShort|Using|Variant|Wend|When|While|Widening|With|WithEvents|WriteOnly|Xor|Region)\b|(#Const|#Else|#ElseIf|#End|#If|#Region)\b",
                    RegexOptions.IgnoreCase | RegexCompiledOption);
        }
...
";
        ...
        protected void InitPHPRegex()
        {
            ...
            PHPCommentRegex3 = new Regex(@"(/\*.*?\*/)|(.*\*/)",
                                         RegexOptions.Singleline | RegexOptions.RightToLeft | RegexCompiledOption);
            PHPVarRegex = new Regex(@"_[a-zA-Z_\d]*\b", RegexCompiledOption);
            PHPKeywordRegex1 =
                new Regex(
                    @"\b(write|writeline|read|readline|scan|scanline)\b",
                    RegexCompiledOption);
            PHPKeywordRegex2 =
                new Regex(
                    @"\b(true|false|if|else|enif|loop|enlp|fasm|enfm|new|proc|ret|start)\b",
                    RegexCompiledOption);
            PHPKeywordRegex3 = new Regex(@"__METHOD__|__NAMESPACE__",
                                         RegexCompiledOption);
        }
...
";
            ...
            //set folding markers
            range.SetFoldingMarkers("<", ">"); //allow to collapse brackets block
            range.SetFoldingMarkers("fasm", "enfm"); //allow to collapse brackets block
            range.SetFoldingMarkers(@"/\*", @"\*/"); //allow to collapse comment block
        }
        ...
        protected void InitJScriptRegex()
        {
            ...
            JScriptKeywordRegex =
                new Regex(
                    @"\b(true|false|if|else|enif|loop|enlp)\b",
                    RegexCompiledOption);
        }
